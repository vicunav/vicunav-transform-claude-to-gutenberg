...
3. Levantar el frontend fuente en un entorno limpio y capturar baseline en
   desktop, tablet y móvil, incluyendo estados interactivos relevantes.
4. Crear el contrato de migración: páginas, secciones, copy, assets, tokens,
   interacciones, fuentes de datos, integraciones y criterios de aceptación.
5. Separar tokens reutilizables de geometría local. Mapear los primeros a
   `theme.json` y conservar la segunda mediante bloques, layouts y clases
   semánticas mínimas.
6. Diseñar la arquitectura FSE: templates, template parts, patterns, contenido
   de páginas y, solo cuando sea necesario, bloques custom o plugins.
7. Implementar primero una sección representativa. Comparar frontend y Site
   Editor antes de escalar el patrón al resto del sitio.
8. Migrar página por página y sección por sección. Mantener el proyecto fuente
   visible como referencia durante toda la implementación.
9. Instalar o activar el theme en LocalWP mediante el flujo documentado del
   proyecto. Crear o actualizar contenido de forma idempotente y preservando lo
   existente.
10. Ejecutar `scripts/validate_fse_theme.mjs <ruta-theme>` y corregir todos los
    errores. Revisar las advertencias aplicables.
11. Completar QA visual, estructural, editorial, responsive, accesible y de
    rendimiento en frontend y Site Editor.
12. Entregar evidencia, limitaciones, rollback y siguiente unidad de trabajo.
